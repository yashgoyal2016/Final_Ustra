...
class Products extends React.Component {
  ...
  incProductCount = () => {
    this.state.productNo === 3 ? (
      this.setState({ productNo: this.state.productNo.length, expanded: true })
    ) : (
      this.setState({ productNo: 3, expanded: false })
    )
  }

showProducts = (category_id,category_name) => { 
  axios.get(`https://backend.ustraa.com/rest/V1/api/catalog/v1.0.1?category_id=${category_id}`).then(res => {
    this.setState({
...
handleClose = () => {
  ...
  render () {
    ...
    const {productsList, anchorEl, heading, open,categoryName, expanded, categoryList, productNo} = this.state;
              ...
              {categoryList && categoryList.map((cat,i) => {
                return (
                  <Grid item lg={2} xs={6} key={i} >
                    <Button style={{margin:'10px', backgroundImage: `url(${cat.category_image})`}} className={classes.categoryButton} variant='contained' onClick={() => this.showProducts(cat.category_id, cat.category_name)}>{cat.category_name}</Button>
                  </Grid>
                )
              })}
              </Grid>
              ...
              <Grid item xs={12} key={i}>
                <Card className={classes.root} elevation={0}>
                    ...
                    <div className={classes.details}>
                      {product.rating ? <div style={{display:'flex', justifyContent:'flex-end', marginBottom:"-10px"}}><Typography variant='subtitle2' style={{color:'grey'}}>{product.rating} <StarRateIcon fontSize='small' style={{verticalAlign:'middle', marginTop:'-5px', marginLeft:'-5px'}} /></Typography></div> : null}
                    
                        <CardContent className={classes.content}>
                        <Typography component="h6" variant="subtitle2" className={classes.price}>
                            {product.name}
                        </Typography>
                        {product.weight === 0  ? null : <Typography variant="body2" component="p" className={classes.weight}>
